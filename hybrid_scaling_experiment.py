#!/usr/bin/env python3
"""
Hybrid ToM Scaling Experiment

This script runs experiments with varying numbers of qubits in hybrid ToM models
to measure how performance and runtime scale with quantum circuit size.
"""
...
def run_single_hybrid_experiment(n_qubits: int, train_loader, val_loader, 
                                max_epochs: int = 20, patience: int = 5, lr: float = 3e-4, device: str = "cpu") -> Dict:
    """Run a single hybrid experiment with specified number of qubits until convergence."""
    print(f"Running hybrid experiment with {n_qubits} qubits...")
...
def run_hybrid_scaling_experiments(qubit_counts: List[int], 
                                  train_loader, val_loader,
                                  max_epochs: int = 20, patience: int = 5, lr: float = 3e-4, 
                                  device: str = "cpu") -> List[Dict]:
    """Run hybrid experiments with varying qubit counts."""
    if not _HAS_PENNYLANE:
        print("ERROR: PennyLane not available. Cannot run hybrid experiments.")
        return []
    ...
    for n_qubits in qubit_counts:
        try:
            result = run_single_hybrid_experiment(n_qubits, train_loader, val_loader, 
                                                max_epochs, patience, lr, device)
            results.append(result)
            print(f"Completed hybrid {n_qubits} qubits: Acc={result['best_overall_acc']:.3f}, "
                  f"Epochs={result['total_epochs']}, Time={result['total_time']:.1f}s\n")
        except Exception as e:
            print(f"ERROR: Failed to run hybrid experiment with {n_qubits} qubits: {e}")
            continue
    
    return results

def plot_hybrid_scaling_results(results: List[Dict], save_path: str = None):
    """Plot hybrid scaling results."""
    if not results:
    ...
    ax2.set_title('Hybrid Runtime Scaling')
...
def print_hybrid_summary_table(results: List[Dict]):
    """Print a summary table of hybrid results."""
    if not results:
...
def main():
    parser = argparse.ArgumentParser(description="Hybrid ToM Scaling Experiment")
    parser.add_argument('--seed', type=int, default=1234, help='Random seed')
    parser.add_argument('--qubits', type=str, default='2,4,6,8', 
    ...
    parser.add_argument('--save-results', type=str, default='hybrid_scaling_results.json',
                       help='File to save results')
    parser.add_argument('--save-plots', type=str, default='hybrid_scaling_plots.png',
    ...
    print(f"Testing hybrid qubit counts: {qubit_counts}")
    
    if not _HAS_PENNYLANE:
        print("ERROR: PennyLane not available. Please install pennylane to run hybrid experiments.")
        return
    ...
    print(f"\nStarting hybrid scaling experiments with {len(qubit_counts)} qubit configurations...")
    results = run_hybrid_scaling_experiments(qubit_counts, train_loader, val_loader,
                                           args.max_epochs, args.patience, args.lr, args.device)
    
    # Display results
    print_hybrid_summary_table(results)
    ...
    # Plot results
    if results:
        plot_hybrid_scaling_results(results, args.save_plots)
    
    print("\nHybrid experiment completed!")

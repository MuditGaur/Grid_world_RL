#!/usr/bin/env python3
"""
Parameter-Matched Belief State Comparison Experiment

This script runs experiments comparing classical and quantum belief state
representations with approximately the same number of parameters (~36K).
This provides a fair comparison by eliminating parameter count as a confounding factor.
"""
...
def run_parameter_matched_experiment(belief_type: str, train_loader, val_loader, 
                                    n_qubits: int = 8, max_epochs: int = 20, 
                                    patience: int = 5, lr: float = 3e-4, 
                                    device: str = "cpu") -> Dict:
    """Run experiment with specified belief state type (parameter-matched)."""
    print(f"Running {belief_type} belief state experiment (parameter-matched)...")
    ...
    results = {
        ...
        'n_qubits': n_qubits if belief_type == "quantum" else 0,
...
def run_parameter_matched_comparison(train_loader, val_loader, n_qubits: int = 8,
                                    max_epochs: int = 20, patience: int = 5, 
                                    lr: float = 3e-4, device: str = "cpu") -> List[Dict]:
    """Run experiments comparing parameter-matched belief state types."""
    ...
    if _HAS_PENNYLANE:
        ...
        print("WARNING: PennyLane not available. Only running parameter-matched classical belief state.")
    
    results = []
    
    for belief_type in belief_types:
        try:
            result = run_parameter_matched_experiment(
            ...
            print(f"Completed {belief_type} belief state: Acc={result['best_overall_acc']:.3f}, "
                  f"Params={result['model_params']}, Epochs={result['total_epochs']}, "
...
def plot_parameter_matched_results(results: List[Dict], save_path: str = None):
    """Plot parameter-matched comparison results."""
    if not results:
    ...
    fig.suptitle('Parameter-Matched Belief State Comparison (~36K Parameters)', fontsize=16)
    ...
    colors = {'classical_matched': 'blue', 'quantum': 'red', 'hybrid_matched': 'green'}
    ...
    ax6.set_title('Model Size (Should be ~36K)')
    ...
    if save_path:
        ...
        print(f"Parameter-matched comparison plots saved to {save_path}")
    plt.show()

def print_parameter_matched_table(results: List[Dict]):
    """Print a parameter-matched comparison table of results."""
    if not results:
        print("No results to display")
        return
    
    print("\n" + "="*120)
    print("PARAMETER-MATCHED BELIEF STATE COMPARISON RESULTS (~36K Parameters)")
    print("="*120)
    print(f"{'Belief Type':<18} {'Qubits':<8} {'Overall Acc':<12} {'FB Acc':<10} {'Vis Acc':<10} "
          f"{'Total Time':<12} {'Epochs':<8} {'Best Epoch':<12} {'Params':<10}")
    print("-"*120)
    
    for r in results:
        print(f"{r['belief_type']:<18} {r['n_qubits']:<8} {r['best_overall_acc']:<12.3f} "
...
def main():
    parser = argparse.ArgumentParser(description="Parameter-Matched Belief State Comparison")
    parser.add_argument('--seed', type=int, default=1234, help='Random seed')
    parser.add_argument('--qubits', type=int, default=8, help='Number of qubits for quantum')
    ...
    parser.add_argument('--p-swap', type=float, default=0.25, help='Probability of swap after subgoal')
    parser.add_argument('--swap-window', type=int, default=10, help='Steps after swap to mark hidden')
    parser.add_argument('--force-swap', action='store_true', help='Force a swap event in each episode')
    parser.add_argument('--force-swap-step', type=int, default=-1, help='Step index to force swap (default: max_steps//3)')
    parser.add_argument('--use-rb-agents', action='store_true', help='Use rule-based agents')
    parser.add_argument('--device', type=str, default='cpu', help='Device to use')
    parser.add_argument('--save-results', type=str, default='parameter_matched_results.json',
                       help='File to save results')
    parser.add_argument('--save-plots', type=str, default='parameter_matched_plots.png',
                       help='File to save plots')
    
    args = parser.parse_args()
    set_seed(args.seed)
    
    print(f"Testing parameter-matched belief state types with ~36K parameters each")
    print(f"Quantum model uses {args.qubits} qubits")
    
    # Build datasets
    print("Building rollout datasets...")
    # Determine forced swap step
    force_step = None if args.force_swap_step < 0 else args.force_swap_step
    ...
    # Run parameter-matched comparison experiments
    print(f"\nStarting parameter-matched belief state comparison experiments...")
    results = run_parameter_matched_comparison(train_loader, val_loader, args.qubits,
                                             args.max_epochs, args.patience, args.lr, args.device)
    
    # Display results
    print_parameter_matched_table(results)
    ...
    # Plot results
    if results:
        plot_parameter_matched_results(results, args.save_plots)
    
    print("\nParameter-matched belief state comparison experiment completed!")
